...
\begin{itemize}
    \item We formulate financial risk scoring as a time-constrained retrieval problem and instantiate it with FRAA, which combines temporal user encoding, timestamp-valid evidence retrieval, cross-attention fusion, and explanation generation.
...
Classical credit-risk systems are built around statistical classifiers, scorecards, and tree ensembles, which remain strong baselines for heterogeneous tabular data \cite{hand1997statistical,thomas2000survey}. Benchmarking studies extend this evidence across credit-scoring datasets and classifier families \cite{baesens2003benchmarking,abdou2011credit}. Later surveys document the breadth of classification methods used in this area \cite{louzada2016classification}. In operational settings, Random Forests and gradient boosting are still widely used because they fit mixed tabular features well \cite{breiman2001random,friedman2001greedy}. XGBoost and large benchmark studies further explain why boosted trees are difficult baselines to displace \cite{chen2016xgboost,lessmann2015benchmarking}. Credit and fraud datasets also tend to be imbalanced and loss-asymmetric, so ranking quality and calibration matter alongside raw classification accuracy \cite{crook2007recent,brown2012experimental}. Consumer-credit and fraud studies make the same point in realistic investigation settings \cite{khandani2010consumer,bellotti2009support}. Fraud-detection reviews connect these models to monitoring workflows \cite{bolton2002statistical,ngai2011application}. Their strength, however, also defines the boundary of this paradigm: the model state is largely determined by engineered feature stores, while newly emerging market or regulatory evidence must be manually transformed before it can affect a decision.
...
\begin{figure}
\centering
\includegraphics[width=0.55\textwidth]{inference_latency_throughput.pdf}
